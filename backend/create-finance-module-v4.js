/**
 * Crear módulo de Finanzas y asignarlo a empresa ISI (v4)
 * Genera UUID para el campo id
 */
...
async function createFinanceModule() {
  try {
    ...
    console.log('   UUID generado:', moduleId);

    const insertResult = await pool.query(
      `INSERT INTO system_modules
       (id, module_key, name, description, category, icon, is_core, module_type, available_in, metadata)
       ...
       RETURNING id, module_key, name, icon`,
      [
        moduleId,
        'finance-dashboard',
        'Finanzas',
    ...
    if (verifyResult.rows.length > 0) {
      ...
      console.log('   💡 Refrescar navegador (Ctrl + Shift + R) para ver el módulo');
    }
