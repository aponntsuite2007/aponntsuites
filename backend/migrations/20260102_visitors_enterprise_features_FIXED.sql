-- ENTERPRISE FEATURES: Visitors Module (CORREGIDA)
-- Mejoras de nivel enterprise para control de visitantes multi-tenant

-- 1. Agregar categoría de visitante
ALTER TABLE visitors
ADD COLUMN IF NOT EXISTS visitor_category VARCHAR(50) DEFAULT 'standard';

COMMENT ON COLUMN visitors.visitor_category IS 'Categoría: vip, contractor, auditor, medical, delivery, standard, other';

-- 2. Agregar número de badge
ALTER TABLE visitors
...
-- 3. Agregar nivel de clearance de seguridad
ALTER TABLE visitors
ADD COLUMN IF NOT EXISTS security_clearance_level INT DEFAULT 1;

COMMENT ON COLUMN visitors.security_clearance_level IS 'Nivel: 1=público, 2=restringido, 3=confidencial, 4=secreto';

-- 4. Agregar audit trail
ALTER TABLE visitors
...
ADD COLUMN IF NOT EXISTS status_updated_by INT REFERENCES users(user_id);

COMMENT ON COLUMN visitors.status_updated_at IS 'Última vez que cambió authorization_status';
COMMENT ON COLUMN visitors.status_updated_by IS 'Quién cambió el status por última vez (user_id)';
...
-- 7. Vista materializada para analytics (CORREGIDA con company_id)
...
VALUES (
  '20260102_visitors_enterprise_features_FIXED',
  NOW(),
  'ENTERPRISE FIXED: visitor_category, badge_number, security_clearance, audit_trail, analytics view'
