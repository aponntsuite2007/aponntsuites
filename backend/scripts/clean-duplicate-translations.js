...
console.log('\n🧹 Limpiando traducciones duplicadas en locales/*.json...\n');

const localesDir = path.join(__dirname, '../public/locales');
const languages = ['es', 'en', 'pt', 'de', 'it', 'fr'];

// Traducciones correctas finales (SIN números al inicio)
...
languages.forEach(lang => {
  ...
  // Leer como texto para detectar duplicados
  const rawContent = fs.readFileSync(filePath, 'utf8');

  // Parse JSON
  let content = JSON.parse(rawContent);

  // Aplicar las traducciones correctas
  Object.keys(correctTranslations[lang]).forEach(key => {
    content[key] = correctTranslations[lang][key];
  });

  // Guardar limpio
  fs.writeFileSync(filePath, JSON.stringify(content, null, 2), 'utf8');

  console.log(`✅ ${lang}.json limpiado y actualizado`);
});

console.log('\n✅ COMPLETADO - Todos los archivos de traducción limpios y consistentes\n');
