/**
 * ============================================================================
 * E2E TEST - Módulos corregidos (hse-deliveries, departments)
 * ============================================================================
 * Test específico para verificar los módulos que fueron corregidos
 * ============================================================================
 */
...
async function takeScreenshot(page, name) {
    try {
        const filename = `fixed-${name}-${Date.now()}.png`;
        const filepath = path.join(CONFIG.screenshotsDir, filename);
        await page.screenshot({ path: filepath, fullPage: true });
        log('info', `Screenshot: ${filename}`);
        return filepath;
    } catch (e) {
        log('warn', `No se pudo tomar screenshot: ${e.message}`);
        return null;
    }
}
...
async function performLogin(page) {
    log('step', 'Login...');
    await page.goto(`${CONFIG.baseUrl}/panel-empresa.html`, { waitUntil: 'networkidle0', timeout: 60000 });
    await wait(2000);
...
async function navigateToModule(page, moduleKey, moduleConfig) {
    ...
    // Determinar la key de navegación (puede ser diferente al moduleKey para sub-módulos)
    const navKey = moduleConfig.menuSelector?.match(/data-module-key='([^']+)'/)?.[1] || moduleKey;
    const navName = moduleConfig.parentModuleName || moduleConfig.name;
...
async function runTest() {
    console.log('\n' + '='.repeat(70));
    console.log(`${colors.bold}E2E TEST - MÓDULOS CORREGIDOS (hse-deliveries, departments)${colors.reset}`);
    ...
    try {
        ...
        // Test módulos corregidos
        const testModules = ['hse-deliveries', 'departments'];
    ...
    // Resumen
    console.log('\n' + '='.repeat(70));
    console.log(`${colors.bold}RESUMEN TEST MÓDULOS CORREGIDOS${colors.reset}`);
    console.log('='.repeat(70));
