...
console.log('\n📝 Insertando módulo internationalization en engineering-metadata.js...\n');

const metadataPath = path.join(__dirname, '../engineering-metadata.js');
let content = fs.readFileSync(metadataPath, 'utf8');

// Buscar el cierre de employee-360 (línea 7539: "    }")
// Necesitamos insertar DESPUÉS de esa línea pero ANTES del cierre de modules
const employee360Pattern = /(    "employee-360": \{[\s\S]*?    \})\n(  \},\n  "workflows":)/;

const internationalizationModule = `,
...
if (!employee360Pattern.test(content)) {
  console.error('❌ No se encontró el patrón de employee-360 → workflows');
  process.exit(1);
}

// Insertar el módulo
content = content.replace(employee360Pattern, `$1${internationalizationModule}\n$2`);

// Guardar
fs.writeFileSync(metadataPath, content, 'utf8');

console.log('✅ Módulo "internationalization" agregado correctamente');
console.log('📍 Ubicación: Después de employee-360, antes de workflows');
console.log('📊 Estado: PRODUCTION - 100% completado');
console.log('🌍 6 idiomas: ES, EN, PT, DE, IT, FR');
console.log('📝 Commit: 6fcdbd9\n');

// Verificar
const verificacion = fs.readFileSync(metadataPath, 'utf8');
if (verificacion.includes('"internationalization":')) {
  console.log('✅ VERIFICADO: Módulo presente en el archivo\n');
} else {
  console.error('❌ ERROR: Módulo NO se encuentra en el archivo\n');
  process.exit(1);
}
