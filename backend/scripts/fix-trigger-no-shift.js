...
console.log('🔧 [FIX] Eliminando LEFT JOIN shifts del trigger (columna no existe)...\n');
...
async function fixTrigger() {
    try {
        ...
        // Actualizar la función eliminando el LEFT JOIN con shifts
    ...
    IF NEW.absence_type IN ('medical_illness', 'work_accident', 'non_work_accident', 'occupational_disease', 'maternity')
       AND NEW.assigned_doctor_id IS NULL THEN
        ...
        IF v_doctor_id IS NOT NULL THEN
            ...
            -- Obtener datos del empleado para la notificación (SIN shift porque no existe en users)
            SELECT
                u."firstName" || ' ' || u."lastName",
                u."employeeId",
                u.dni,
                d.name
            INTO v_employee_name, v_employee_legajo, v_employee_dni, v_department
            FROM users u
...
$$ LANGUAGE plpgsql;
        ...
        console.log('   ✓ Eliminado: LEFT JOIN shifts (columna no existe en users)');
        console.log('   ✓ Eliminado: v_shift variable\n');
