...
// Leer, modificar y escribir de una sola vez
...
const searchPattern = `// GET /api/v1/legal/cases/:id/deadlines - Obtener vencimientos del caso
router.get('/cases/:id/deadlines'`;

const insertCode = `// POST /api/v1/legal/documents/:docId/response - Registrar respuesta/acuse
...
if (content.includes(searchPattern)) {
    content = content.replace(searchPattern, insertCode + searchPattern);
    fs.writeFileSync(path, content, 'utf8');
    console.log('✅ Endpoints agregados correctamente');
} else {
    console.log('❌ Patrón no encontrado');
    // Intentar buscar variantes
    const lines = content.split('\n');
    for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes('cases/:id/deadlines') && lines[i].includes('GET')) {
            console.log(`Encontrado en línea ${i + 1}: ${lines[i].substring(0, 80)}`);
        }
    }
}
