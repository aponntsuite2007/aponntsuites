...
// ⭐ FIX 24: Esperar a que el elemento sea clickable (no solo enabled)      console.log('    ⏳ Esperando que #passwordInput sea clickable...');      await this.page.waitForFunction(        () => {          const el = document.querySelector('#passwordInput');          if (!el) return false;          const rect = el.getBoundingClientRect();          // Verificar que esté visible y no cubierto          return rect.width > 0 && rect.height > 0 && el.offsetParent !== null;        },        { timeout: 10000 }      );
