  ...
  try {
    ...
    if (bestMatch && bestSimilarity >= threshold) {
      ...
      try {
        ...
        if (!todayAttendance) {
          ...
          if (authCheck.wasAuthorized) {
            ...
            const [insertResult] = await sequelize.query(`
              ...
              VALUES (gen_random_uuid(), CURRENT_DATE, :userId, :checkInTime, :checkInMethod, :status, NOW(), NOW())
          ...
          // Tiene registro hoy pero SIN checkOutTime → VERIFICAR TIEMPO MÍNIMO
          ...
          // ES SALIDA (después de validar tiempo mínimo)
          operationType = 'clock_out';
          attendanceId = todayAttendance.id;
          console.log(`📤 [AUTO] SALIDA detectada para ${bestMatch.employeeName} (Attendance ID: ${attendanceId})`);
        ...
        console.log(`✅ [AUTO] ${operationType.toUpperCase()} registrado: ${bestMatch.employeeName} - ${timestamp}`);

        // Respuesta mínima para semáforo (SIN mensajes)
        return res.json({
          success: true,
...
async function checkLateArrivalAuthorization(employeeId, companyId) {
  ...
  try {
    // 1. Obtener datos del empleado con shift y departamento
    ...
    // 2. Si no tiene shift asignado, permitir ingreso sin restricción
    if (!employeeData.shift_id || !employeeData.starttime) {
      console.log(`✅ [LATE-CHECK] Employee ${employeeData.first_name} ${employeeData.last_name} has no shift - allowing entry`);
