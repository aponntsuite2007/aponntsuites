/**
 * CONFIGURACIÓN E2E - GESTIÓN DE CONSENTIMIENTOS (ADMIN)
...
module.exports = {
  ...
  moduleKey: 'admin-consent-management',
  moduleName: 'Gestión de Consentimientos (Admin)',
  category: 'empresa',
  baseUrl: 'http://localhost:9998/panel-empresa.html',

  navigation: {
    listContainerSelector: '.users-table',
  ...
  database: {
    table: 'admin-consent-management',
    primaryKey: 'id',
    ...
    testDataCleanup: async (db, id) => {
      // await db.query('DELETE FROM admin-consent-management WHERE id = $1', [id]);
    }
  },
