/**
 * CONFIGURACIÓN E2E - DEPLOY MANAGER 3-STAGE
 * ⭐ AUTO-GENERADO por inspector de frontend
 * Sistema SYNAPSE - Selectores REALES extraídos del código
 */

module.exports = {
  moduleKey: 'deploy-manager-3stages',
  moduleName: 'Deploy Manager 3-Stage',
  category: 'empresa',
  baseUrl: 'http://localhost:9998/panel-empresa.html',

  navigation: {
    listContainerSelector: '#deploy-manager-3stagesContainer, #mainContent',
  ...
  database: {
    ...
    testDataCleanup: async (db, id) => {
      // await db.query('DELETE FROM deploy-manager-3stages WHERE id = $1', [id]);
    }
  },
