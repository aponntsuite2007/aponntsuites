/**
 * CONFIGURACIÓN E2E - SINCRONIZACIÓN DE DEPLOY
 * ⭐ AUTO-GENERADO por inspector de frontend
 * Sistema SYNAPSE - Selectores REALES extraídos del código
 */

module.exports = {
  moduleKey: 'deployment-sync',
  moduleName: 'Sincronización de Deploy',
  category: 'empresa',
  baseUrl: 'http://localhost:9998/panel-empresa.html',

  navigation: {
    listContainerSelector: '.deploy-sync-container',
  ...
  database: {
    ...
    testDataCleanup: async (db, id) => {
      // await db.query('DELETE FROM deployment-sync WHERE id = $1', [id]);
    }
  },
