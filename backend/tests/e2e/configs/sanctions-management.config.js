/**
 * CONFIGURACIÓN E2E - GESTIÓN DE SANCIONES
...
module.exports = {
  ...
  moduleName: 'Gestión de Sanciones',
  category: 'empresa',
  baseUrl: 'http://localhost:9998/panel-empresa.html',

  navigation: {
    listContainerSelector: '.sanctions-container',
    createButtonSelector: '.btn-new',
    openModalSelector: 'button[data-action="open"], .btn-new',
  ...
  database: {
    ...
    testDataCleanup: async (db, id) => {
      // await db.query('DELETE FROM sanctions-management WHERE id = $1', [id]);
    }
  },
